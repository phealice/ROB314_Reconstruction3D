...
class YoloModel():
    ...
    '''
    def __init__(self):
        current_path="/home/alice/catkin_ws/src/yolo/src/"
...
class YoloDetectPublisher():
    ...
    def __init__(self):
        ...
        self.yolo = YoloModel()
        # Define subscribers
        self.image_sub = rospy.Subscriber("/tello/image_decompressed", Image, self.callback, queue_size=1000)

        # Define publishers
        self.pub_ = rospy.Publisher("/Yolo_Result", Image, queue_size=1)
        rospy.loginfo("Launched node for object detection")

    def callback(self,image_msg):
        cv_image = self._cv_bridge.imgmsg_to_cv2(image_msg, "rgb8")
        result=self.yolo.inference(cv_image)
	cv2.imshow("result yolo",result)
	cv2.waitKey(1)
